...
/**
 * V2 of the ML Kit signal-fusion detector.
 *
 * Gaps closed vs [MlKitSignalFusionQuestionDetector]:
 *   A1 — anchor match/mismatch signals ported from [SignalFusionQuestionDetector].
 *   A2 — gap multipliers aligned to legacy [SignalFusionQuestionDetector] (1.5×/3.0×).
 *   A4 — high-confidence threshold (0.85), modal-margin bin size (5 px), winsorization disabled.
 */
class MlKitSignalFusionQuestionDetectorV2 @JvmOverloads constructor(
    ...
    private fun detectFromGeoms(
        ...
        val anchorProfile = buildAnchorProfile(sorted, calibration)
        val signals = buildSignals(calibration, anchorProfile)
    ...
    // ── A1: Anchor profile construction ───────────────────────────────────────
    // Mirrors SignalFusionQuestionDetector calibration Pass 1, operating on the
    // already-normalized MlKitLineGeom list rather than raw OcrResult boxes.

    private fun buildAnchorProfile(
        sorted: List<MlKitLineGeom>,
        calibration: MlKitCalibrationData
    ): SignalFusionQuestionDetector.AnchorProfile? {
        if (calibration.status != MlKitCalibrationStatus.FULL) return null

        val highConf = sorted.filter {
            !it.normalized.score.isNaN() &&
                it.normalized.score >= CALIBRATION_PROFILE.highConfThreshold
        }
        val candidates = highConf.filter {
            SignalFusionQuestionDetector.ANY_NUMBERED_PATTERN
                .containsMatchIn(it.normalized.text.trim())
        }
        if (candidates.size < SignalFusionQuestionDetector.ANCHOR_MIN_COUNT) return null

        val style = detectStyle(candidates.first().normalized.text)
        val observedNumbers = candidates
            .mapNotNull { extractInteger(it.normalized.text, style) }
            .sorted().distinct()
        val leftMin = candidates.minOf { it.rect.left }
        val leftMax = candidates.maxOf { it.rect.left }
        val confidence = minOf(
            1.0f,
            candidates.size.toFloat() / SignalFusionQuestionDetector.ANCHOR_CONFIDENCE_FULL
        )
        return SignalFusionQuestionDetector.AnchorProfile(
            leftMarginRange = leftMin..leftMax,
            style = style,
    ...
    private fun buildSignals(
        calibration: MlKitCalibrationData,
        anchorProfile: SignalFusionQuestionDetector.AnchorProfile?
    ): List<Signal> {
        ...
        signals += Signal(name = "gap_small", score = -3) { ctx, cal ->
            ctx.gapAbove != Float.MAX_VALUE &&
                ctx.gapAbove < cal.medianLineHeight * GAP_SMALL_MULTIPLIER
        }
        ...
        signals += Signal(name = "numbered_marker", score = 9) { ctx, _ ->
            SignalFusionQuestionDetector.ANY_NUMBERED_PATTERN
                .containsMatchIn(ctx.line.text.trim())
        }
        ...
        signals += Signal(name = "marks_tag_prev", score = 3) { ctx, _ ->
            ctx.prevLine != null &&
                SignalFusionQuestionDetector.MARKS_TAG_PATTERN
                    .containsMatchIn(ctx.prevLine.text.trim())
        }

        // ── A1: Anchor profile boost / penalty (FULL status only) ─────────────
        // Score formula mirrors legacy: boost = max(1, round(2 × confidence)).
        if (calibration.status == MlKitCalibrationStatus.FULL && anchorProfile != null) {
            val boost = maxOf(1, (2f * anchorProfile.confidence).roundToInt())
            signals += Signal(name = "anchor_match", score = boost) { ctx, _ ->
                matchesAnchorProfile(ctx.line.text, ctx.rect.left, anchorProfile)
            }
            signals += Signal(name = "anchor_mismatch", score = -3) { ctx, _ ->
                looksLikeNumberedButMismatch(ctx.line.text, ctx.rect.left, anchorProfile)
            }
        }
        ...
        if (calibration.status != MlKitCalibrationStatus.GEOMETRY_ONLY) {
            ...
            signals += Signal(name = "interrogative_word", score = 4) { ctx, _ ->
                ...
                when (lang) {
                    ...
                    else -> EN_INTERROGATIVE.containsMatchIn(t) ||
                        EN_INTERROGATIVE_PHRASE.containsMatchIn(t) ||
    ...
    private fun isQuestionStart(
        ...
        val hasSubQuestion =
            SignalFusionQuestionDetector.SUB_QUESTION_PATTERN
                .containsMatchIn(ctx.line.text.trim())
        debugLog(
    ...
    companion object {
        private const val DEBUG_TAG = "MlKitQDetectorV2"
        ...
        // A2 + A4: calibration profile aligned to legacy SignalFusionQuestionDetector.
        // gapLarge/VeryLarge:   1.5× / 3.0× (MlKit default: 1.4× / 2.8×).
        // highConfThreshold:    0.85  (MlKit default: 0.75).
        // marginBinSizePx:      5 px  (MlKit default: 6 px).
        // winsorize [0, 1]:     disables outlier removal — legacy uses raw heights.
        private val CALIBRATION_PROFILE = MlKitCalibrationProfile(
            gapLargeMultiplier = 1.5f,
            gapVeryLargeMultiplier = 3.0f,
            highConfThreshold = 0.85f,
            marginBinSizePx = 5f,
            winsorizeLowPercentile = 0f,
            winsorizeHighPercentile = 1f
        )

        // ── A1: Anchor helpers ────────────────────────────────────────────────
        // Pure functions ported from SignalFusionQuestionDetector.
        // STYLE_PATTERNS and ANY_NUMBERED_PATTERN are reused directly (internal visibility).

        private fun detectStyle(
            text: String
        ): SignalFusionQuestionDetector.NumberedMarkerStyle =
            SignalFusionQuestionDetector.STYLE_PATTERNS.entries
                .firstOrNull { (_, regex) -> regex.containsMatchIn(text.trim()) }
                ?.key ?: SignalFusionQuestionDetector.NumberedMarkerStyle.NUMERIC_DOT

        private fun extractInteger(
            text: String,
            style: SignalFusionQuestionDetector.NumberedMarkerStyle
        ): Int? {
            val t = text.trim()
            val raw: Int? = when (style) {
                SignalFusionQuestionDetector.NumberedMarkerStyle.NUMERIC_DOT,
                SignalFusionQuestionDetector.NumberedMarkerStyle.NUMERIC_PAREN,
                SignalFusionQuestionDetector.NumberedMarkerStyle.NUMERIC_ENCLOSED_PAREN ->
                    Regex("""\d{1,3}""").find(t)?.value?.toIntOrNull()

                SignalFusionQuestionDetector.NumberedMarkerStyle.NUMERIC_FULLWIDTH_PAREN ->
                    Regex("""\d{1,3}""").find(t.trimStart('（', '('))?.value?.toIntOrNull()

                SignalFusionQuestionDetector.NumberedMarkerStyle.CIRCLED_NUMBER ->
                    if (t.isNotEmpty() && t[0] in '①'..'⑳') t[0].code - '①'.code + 1 else null

                SignalFusionQuestionDetector.NumberedMarkerStyle.Q_PREFIX ->
                    Regex("""(?i)Q\.?(\d{1,3})""").find(t)?.groupValues?.get(1)?.toIntOrNull()

                SignalFusionQuestionDetector.NumberedMarkerStyle.JP_MONDAI ->
                    Regex("""問題(\d{1,3})""").find(t)?.groupValues?.get(1)?.toIntOrNull()

                SignalFusionQuestionDetector.NumberedMarkerStyle.JP_SETSUMON ->
                    Regex("""設問(\d{1,3})""").find(t)?.groupValues?.get(1)?.toIntOrNull()

                SignalFusionQuestionDetector.NumberedMarkerStyle.JP_MON ->
                    Regex("""問(\d{1,3})""").find(t)?.groupValues?.get(1)?.toIntOrNull()

                SignalFusionQuestionDetector.NumberedMarkerStyle.DAI_MON ->
                    Regex("""第(\d{1,3})問""").find(t)?.groupValues?.get(1)?.toIntOrNull()
            }
            return raw?.takeIf { it in 1..200 }
        }

        private fun matchesAnchorProfile(
            text: String,
            left: Float,
            profile: SignalFusionQuestionDetector.AnchorProfile
        ): Boolean {
            val t = text.trim()
            val stylePattern =
                SignalFusionQuestionDetector.STYLE_PATTERNS[profile.style] ?: return false
            ...
            if (left !in xLow..xHigh) return false
        ...
        private fun looksLikeNumberedButMismatch(
            text: String,
            left: Float,
            profile: SignalFusionQuestionDetector.AnchorProfile
        ): Boolean =
            SignalFusionQuestionDetector.ANY_NUMBERED_PATTERN.containsMatchIn(text.trim()) &&
                !matchesAnchorProfile(text, left, profile)
        ...
        private val EN_CONDITIONAL =
            Regex("""(?i)^(?:Given|If|For|When|Assuming|Let|Suppose)\b.+\b($EN_VERBS)\b""")

        private val JA_KEYWORD = Regex("""^(?:問題|設問|問)\s*\d*""")
        private val JA_INTERROGATIVE =
            Regex("""^(?:何|どれ|どの|どう|どのよう|なぜ|どうして|いつ|どこ|だれ|誰|いくつ|いくら)""")
        private val JA_QUESTION_ENDING =
            Regex("""(?:ですか|ますか|でしょうか|だろうか|かな)[。？]?${'$'}""")
        ...
        private val JA_CONDITIONAL =
            Regex("""(?:のとき|ならば?|とすれば|において|の場合|を仮定して|とする).+?(?:$JA_VERBS)[。]?${'$'}""")
    }
}
